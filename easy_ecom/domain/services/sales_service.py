...
class SalesService:
    ...
    def list_draft_orders(self, client_id: str) -> pd.DataFrame:
        ...
        if orders.empty:
            return pd.DataFrame(columns=["order_id", "client_id", "timestamp", "customer_id", "status", "subtotal", "discount", "tax", "grand_total", "note", "item_count"])
        ...
        if items.empty:
            scoped["item_count"] = 0
            return scoped
        counts = items.groupby("order_id", as_index=False).size().rename(columns={"size": "item_count"})
    ...
    def confirm_order(self, order_id: str, user_ctx: dict[str, str]) -> dict[str, str]:
        ...
        if order.get("status") != "draft":
            ...
            existing = invoices[invoices["order_id"] == order_id] if not invoices.empty else pd.DataFrame()
            invoice_no = existing.iloc[0]["invoice_no"] if not existing.empty else ""
            raise ValueError(f"Order already {order.get('status')} {invoice_no}".strip())
        ...
        for row in items.itertuples(index=False):
            qty = float(row.qty)
            price = float(row.unit_selling_price)
            item = SaleItem(product_id=str(row.product_id), qty=qty, unit_selling_price=price)
        ...
        customer_snapshot = self.customers_repo.get(str(order.get("customer_id", ""))) if self.customers_repo else None
        customer_snapshot = customer_snapshot or {}
        shipment_id = new_uuid()
        shipment_no = self.seq_service.next(client_id, "SHIPMENT", year, "SHP")
        self.shipments_repo.append({"shipment_id": shipment_id, "client_id": client_id, "shipment_no": shipment_no, "order_id": order_id, "customer_id": order.get("customer_id", ""), "timestamp": now_iso(), "status": "packed", "ship_to_name_snapshot": customer_snapshot.get("full_name", ""), "ship_to_phone_snapshot": customer_snapshot.get("phone", ""), "ship_to_address_snapshot": customer_snapshot.get("address_line1", ""), "courier": "", "tracking_no": ""})
        ...
        idx = orders[orders["order_id"] == order_id].index
        if len(idx) == 0:
            raise ValueError("Order not found during confirmation")
        i = idx[0]
