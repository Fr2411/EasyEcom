...
class InventoryService:
    ...
    @staticmethod
    def build_product_id(product_name: str, lot_id: str) -> str:
        normalized_name = "_".join(product_name.strip().split())
        return f"{normalized_name}_{lot_id}"
    ...
    def available_qty(self, client_id: str, product_name: str) -> float:
        lots = self.stock_by_lot(client_id)
        if lots.empty:
            return 0.0
        return float(lots[lots["product_name"] == product_name]["qty"].sum())

    def allocate_fifo(self, client_id: str, product_name: str, qty: float) -> list[dict[str, float | str]]:
        if qty <= 0:
            raise ValueError("qty must be positive")
        lots = self.stock_by_lot(client_id)
        product_lots = lots[lots["product_name"] == product_name].sort_values("lot_id")
        remaining = qty
