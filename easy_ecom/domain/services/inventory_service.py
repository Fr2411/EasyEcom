...
class InventoryService:
    ...
    def add_stock(self, client_id: str, product_id: str, qty: float, unit_cost: float, supplier_snapshot: str, note: str, source_type: str = "purchase", source_id: str = "") -> str:
        if qty <= 0 or unit_cost <= 0:
            raise ValueError("qty and unit_cost must be > 0")
        lot_id = self.seq_service.next(client_id, "LOT", pd.Timestamp.utcnow().year, "LOT")
        self.repo.append({"txn_id": new_uuid(), "client_id": client_id, "timestamp": now_iso(), "txn_type": "IN", "product_id": product_id, "qty": str(qty), "unit_cost": str(unit_cost), "total_cost": str(qty * unit_cost), "supplier_snapshot": supplier_snapshot, "note": note, "source_type": source_type, "source_id": source_id, "lot_id": lot_id})
        return lot_id

    def stock_by_lot(self, client_id: str) -> pd.DataFrame:
        df = self.repo.all()
        if df.empty:
            return pd.DataFrame(columns=["product_id", "lot_id", "qty", "unit_cost"])
        ...
        g = d.groupby(["product_id", "lot_id"], as_index=False).agg({"signed_qty": "sum", "unit_cost": "last"})
    ...
    def allocate_fifo(self, client_id: str, product_id: str, qty: float) -> list[dict[str, float | str]]:
        ...
        for _, row in product_lots.iterrows():
            ...
            if take > 0:
                allocations.append({"lot_id": row["lot_id"], "qty": take, "unit_cost": float(row["unit_cost"])})
                remaining -= take
        if remaining > 0 and not settings.allow_backorder:
            raise ValueError("Insufficient stock")
        return allocations

    def deduct_stock(self, client_id: str, product_id: str, qty: float, source_type: str, source_id: str, note: str = "") -> list[dict[str, float | str]]:
        allocations = self.allocate_fifo(client_id, product_id, qty)
        for alloc in allocations:
            q = float(alloc["qty"])
            uc = float(alloc["unit_cost"])
            self.repo.append({"txn_id": new_uuid(), "client_id": client_id, "timestamp": now_iso(), "txn_type": "OUT", "product_id": product_id, "qty": str(q), "unit_cost": str(uc), "total_cost": str(q * uc), "supplier_snapshot": "", "note": note, "source_type": source_type, "source_id": source_id, "lot_id": str(alloc["lot_id"])})
        return allocations
