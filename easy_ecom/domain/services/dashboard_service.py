...
class DashboardService:
    def __init__(
        ...
        self.metrics = MetricsService(inv, ledger, orders, invoices, payments or PaymentsRepo(inv.store), order_items, products)

    def kpis(self, client_id: str | None) -> dict[str, float]:
        mtd = DateRange(start=self.metrics.month_start(), end=pd.Timestamp.utcnow().tz_localize(None))
        stock_value = float(self.metrics.current_stock_value_by_product(client_id)["stock_value"].sum())
    ...
    def revenue_trend(self, client_id: str | None, freq: str, start_date: pd.Timestamp, end_date: pd.Timestamp) -> pd.DataFrame:
    ...
    def revenue_by_client(self) -> pd.DataFrame:
        ...
        return d.merge(self.clients.all()[["client_id", "business_name"]], on="client_id", how="left")

    def inventory_value_by_client(self) -> pd.DataFrame:
        rows = []
        for cid in self.clients.all().get("client_id", pd.Series(dtype=str)).tolist():
            stock = self.metrics.current_stock_value_by_product(cid)
            rows.append({"client_id": cid, "stock_value": float(stock["stock_value"].sum()) if not stock.empty else 0.0})
        d = pd.DataFrame(rows)
        if d.empty:
    ...
    def client_health_flags(self) -> pd.DataFrame:
        ...
        for _, c in clients.iterrows():
            ...
            rows.append({"client_id": c["client_id"], "business_name": c["business_name"], "flags": ", ".join(warnings) if warnings else "healthy"})
