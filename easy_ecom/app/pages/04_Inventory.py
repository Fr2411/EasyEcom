...
if parent_name:
    ...
    with st.form("stock_in_variants"):
        ...
        for variant in variants:
            ...
            unit_cost = common_cost if same_cost else c3.number_input("Unit cost", min_value=0.01, key=f"cost_{variant['variant_id']}")
            entries.append((variant, qty, unit_cost))
        submit = st.form_submit_button("Add stock")
    if submit:
        for variant, qty, unit_cost in entries:
            if qty > 0:
                svc.add_stock(client_id, variant["variant_id"], variant["variant_name"], float(qty), float(unit_cost), "", "", user_id=user_id)
        st.success("Stock posted")

st.dataframe(svc.stock_by_lot(client_id), use_container_width=True)
...
if not pricing_products.empty:
    selected = st.selectbox("Select product to edit pricing", [f"{r.product_id} - {r.product_name}" for r in pricing_products.itertuples(index=False)])
