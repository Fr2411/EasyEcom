...
def create_sale(
    ...
    try:
        result = service.create_sale(
            ...
            lines=[SalesLineInput(product_id=line.product_id, qty=line.qty, unit_price=line.unit_price) for line in payload.lines],
    ...
    if service is not None:
        try:
            result = service.create_sale(
                ...
                user_id=user.user_id,
                customer_id=payload.customer_id,
                lines=[SalesLineInput(product_id=line.product_id, qty=line.qty, unit_price=line.unit_selling_price) for line in payload.items],
                discount=payload.discount,
                tax=payload.tax,
                note=payload.note,
            )
